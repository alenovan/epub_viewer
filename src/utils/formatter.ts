...
export function formatJumlahTerjual(pembelian: number): string {
  const dataPembelian = pembelian;
  if (pembelian >= 100000000) {
    const jutaan: string[] = ["", "JT+", "JT+", "JT+"];
    const digit: number = Math.floor(Math.log10(pembelian) / Math.log10(1000));
    const jumlahTersingkat: string = (pembelian / 10 ** 6).toFixed(0) + jutaan[digit];
    return jumlahTersingkat;
  }
  if (pembelian >= 1000000) {
    const jutaan: string[] = ["", "JT+", "JT+", "JT+"];
    const digit: number = Math.floor(Math.log10(dataPembelian) / Math.log10(1000));
    const jumlahTersingkat: string = (dataPembelian / 10 ** (3 * digit)).toFixed(1) + jutaan[digit];
    return jumlahTersingkat;
  }
  if (pembelian >= 10100) {
    const ribuan: string[] = ["", "RB+", "RB+", "RB+"];
    const digit: number = Math.floor(Math.log10(pembelian) / Math.log10(1000));
    const jumlahTersingkat: string = (pembelian / 10 ** (3 * digit)).toFixed(1) + ribuan[digit];
    return jumlahTersingkat;
  }
  // Check if the number is an integer
  if (Number.isInteger(pembelian)) {
    return `${pembelian.toLocaleString("en-US", { minimumFractionDigits: 0 })}`;
  }
  return `${pembelian.toString()}`;
}
